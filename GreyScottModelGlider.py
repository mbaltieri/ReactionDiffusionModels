...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 28 17:52:30 2017
...
dpi = 36.0
figsize= 4*size[1]/float(dpi),4*size[0]/float(dpi)
...
#            F = .057
        ...
        if i % 25 == 0:
            a = int(np.random.normal(centroid[0],10,1))
            b = int(np.random.normal(centroid[1]+5,10,1))
            V[a:a+1,b:b+1] = 0.0
...
#        # find centroid of spot
        coord = np.argwhere(binary_v[:,30:60]>.5)
        length = coord.shape[0]
        centroid[0] = np.sum(coord[:,0])/length
        centroid[1] = np.sum(coord[:,1])/length+30
#        
#        # find membrane
...
### predictive information + active information storage analysis
...
#piLocalV = np.zeros((runs,size,size,iterations-starting_point_time_series))
#
## chemical U
#piAverageU = np.zeros((runs,size,size))
#piLocalU = np.zeros((runs,size,size,iterations-starting_point_time_series))
...
##            sourceArray = v_hist[0,starting_point_time_series:,i,j].tolist()            # entire grid
...
#
## chemical V
#piAverageV_avg = np.average(piAverageV, axis=0)
#fig = plt.figure()
...
#ax = plt.contourf(x, y, piAverageV_avg, 50, cmap=cm.jet)
